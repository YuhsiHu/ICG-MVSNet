...
parser.add_argument('--lrepochs', type=str, default="1,3,5,7,9,11,13:1.5", help='epoch ids to downscale lr and the downscale rate')
...
if __name__ == '__main__':
    ...
    if args.dataset.startswith('dtu'):
        train_dataset = MVSDataset(args.trainpath, args.trainlist, "train", args.nviews, args.interval_scale, rt=args.rt, use_raw_train=args.use_raw_train)
        test_dataset = MVSDataset(args.testpath, args.testlist, "val", args.nviews, args.interval_scale)
    elif args.dataset.startswith('blendedmvs'):
        train_dataset = MVSDataset(args.trainpath, args.trainlist, "train", args.nviews, robust_train=args.rt)
        test_dataset = MVSDataset(args.testpath, args.testlist, "val", args.nviews)
    if is_distributed:
